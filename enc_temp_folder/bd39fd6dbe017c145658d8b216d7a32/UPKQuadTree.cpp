...
void UPKQuadTree::Subdivide(PKQuadTreeNode* Node)
{
	...
	if (Node == RootNode)
	{
		Node->Children[0]->FrictionCoefficient = 1.1f; // BL
		Node->Children[1]->FrictionCoefficient = 2.0f; // TL
		Node->Children[2]->FrictionCoefficient = 1.3f; // BR
		Node->Children[3]->FrictionCoefficient = 0.95f; // TR
	}
	else
	{
		for (int i = 0; i < 4; ++i)
